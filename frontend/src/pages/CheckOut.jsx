...
import { addMyOrder, setDeliveryFee } from '../redux/userSlice.js';
...
function CheckOut() {
  ...
  const { cartItems, totalAmount, userData, deliveryFee } = useSelector(state => state.user)
  ...
  // const totalAmountRounded = Number(totalAmount || 0) + Number(deliveryFee || 0) + Number(tips || 0);
  // const AmountWithDeliveryFee = totalAmountRounded.toFixed(2);

  useEffect(() => {
    const fee = totalAmount > 500 ? (totalAmount / 20) : ((totalAmount / 20) + 10);
    dispatch(setDeliveryFee(fee));   
  }, [totalAmount]);

 

  const totalAmountRounded = Number(totalAmount || 0) + Number(deliveryFee || 0) 
  const AmountWithDeliveryFee = totalAmountRounded.toFixed(2);
...
// import { serverUrl } from '../App';
// import { addMyOrder, setDeliveryFee } from '../redux/userSlice.js';
// import TipsInput from './TipsInput';   // ✅ import tips component
...
//   const { location, address } = useSelector(state => state.map)
//   const { cartItems, totalAmount, userData, deliveryFee } = useSelector(state => state.user)
//   const tips = useSelector((state) => state.tips.value)   // ✅ tips from redux
//   const [addressInput, setAddressInput] = useState("")
//   const [paymentMethod, setPaymentMethod] = useState("cod")
//   const navigate = useNavigate()
//   const dispatch = useDispatch()
...
//         totalAmount: AmountWithDeliveryFee,
//         cartItems,
//         tips   // ✅ send tips along with order 
//       }, { withCredentials: true })
